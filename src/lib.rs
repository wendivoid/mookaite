...
pub fn run_mookaite(logger: slog::Logger,
            ...
            let mut m = Mookaite::new(logger, Duration::new(reload_time as u64,0), dir, mode, timeout, keep_going);
            m.run();
}
...
impl Mookaite {
    pub fn new(logger: slog::Logger, reload_time: Duration,
        ...
        let xx = XWrapper::new(logger.clone());
        Mookaite {
    ...
    pub fn run_mapped(&mut self) {
        ...
        loop {
            ...
            let _ = self.x.next_event();
            let ref current_bg = self.image_map[self.x.get_current_desktop()];
    ...
    pub fn run_random(&mut self) {
        ...
        loop {
            ...
            let _ = self.x.next_event();
            self.x.change_background(&self.img_dir.0.random_selection().to_path_buf());
        }
    }
